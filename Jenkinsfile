            ...
            steps {
                script {
                    withCredentials([sshUserPrivateKey(credentialsId: 'SSHtoken', keyFileVariable: 'Key')]) {
                        ...
                        sh '''
                        scp -o StrictHostKeyChecking=no -i $Key nodejs-docker-app.tar ec2-user@15.207.72.81:/home/ec2-user/
                        '''

                        // SSH into the EC2 instance, load the image, and run the container
                        sh '''
                        ssh -o StrictHostKeyChecking=no -i $Key ec2-user@15.207.72.81 << 'ENDSSH'
